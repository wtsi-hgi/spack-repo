...
while (( index < TOTAL )); do
  ...
  for (( i=0; i<launch_count; i++ )); do
    ...
    if [[ -n "${DRY_RUN:-}" ]]; then
      ...
      nohup bash -c \
        "cursor-agent -p \"Following the autospack instructions, try installing and validating ${pkg}^r@4.5: (with R version >=4.5) and debug the errors iteratively, until it succeeds. ${PROMPT}\" --model \"gpt-5\" --output-format text -f" \
        >"$log_out" 2>"$log_err" &
    fi
  done
