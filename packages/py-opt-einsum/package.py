...
class PyOptEinsum(PythonPackage):
    ...
    pypi = "opt_einsum/opt_einsum-3.4.0.tar.gz"
    ...
    depends_on("python@3.8:", type=("build", "run"), when="@3.4.0:")
    depends_on("python@3.5:", type=("build", "run"), when="@:3.3.0")
    depends_on("py-setuptools", type="build")
    depends_on("py-numpy@1.7:", type=("build", "run"))
    depends_on("py-hatchling", type="build", when="@3.4.0:")
    depends_on("py-hatch-vcs", type="build", when="@3.4.0:")
    depends_on("py-hatch-fancy-pypi-readme@22.5:", type="build", when="@3.4.0:")

    @when("^python@3.12:")
    def patch(self):
        if os.path.exists("versioneer.py"):
            filter_file("configparser.SafeConfigParser()", "configparser.ConfigParser()", "versioneer.py", string=True)
