...
class PyMultiqc(PythonPackage):
    ...
    def patch(self):
        # MultiQC v1.18-1.23 import external pyaml_env; older Spack package name is
        # 'pyyaml-env-tag'. Add shim only for those versions.
        if self.spec.satisfies("@1.18:1.23"):
            filter_file(
                "import pyaml_env",
                "import yaml_env_tag as pyaml_env",
                "multiqc/config.py",
            )
        # Only replace parse_config for older MultiQC versions that call it
