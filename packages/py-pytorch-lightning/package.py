...
class PyPytorchLightning(PythonPackage):
    ...
    version("1.5.8", sha256="57e7c9ea3663e5d8416be6a340d3a8d9271eb105fba82b7f7497cf0299880939")
    ...
    depends_on("py-setuptools@:66", when="@:1.8.3", type=("build", "run"))
    ...
    depends_on("py-torch@1.8:", when="@1.5.8:", type=("build", "run"))
    depends_on("py-torch@1.6:", when="@1.4:1.5.3", type=("build", "run"))
    ...
    @when("@1.5.8")
    def patch(self):
        filter_file("1.7.*", "1.7.0", "requirements.txt", string=True)
        filter_file("0.8.*", "0.8.0", "requirements/extra.txt", string=True)
        filter_file("0.8.*", "0.8.0", "requirements/examples.txt", string=True)
