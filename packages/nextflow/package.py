...
class Nextflow(Package):
    ...
    def install_test(self):
        with working_dir("spack-test", create=True):
            env = os.environ.copy()
            java_bin = join_path(self.spec["java"].prefix, "bin")
            env["JAVA_HOME"] = str(self.spec["java"].prefix)
            env["PATH"] = env.get("PATH", "")
            env["PATH"] = "{}{}{}".format(java_bin, os.pathsep, env["PATH"])
            Executable(join_path(self.prefix.bin, "nextflow"))("-version", env=env)
