...
class Poppler(CMakePackage):
    ...
    variant("gpg", default=False, description="Enable GPG cryptographic backend")
    ...
    depends_on("gpgme", when="@:21")
    depends_on("gpgme", when="@22:+gpg")
    ...
    def cmake_args(self):
        ...
        if spec.satisfies("@22:"):
            if "+cms" in spec:
                args.append("-DENABLE_LCMS=ON")
            else:
                args.append("-DENABLE_LCMS=OFF")
        else:
            if "+cms" in spec:
        ...
        if spec.satisfies("@22:"):
            if "+gpg" in spec:
                args.append("-DENABLE_GPGME=ON")
            else:
                args.append("-DENABLE_GPGME=OFF")
        ...
        if spec.satisfies("@22:"):
            if "+tiff" in spec:
                args.append("-DENABLE_LIBTIFF=ON")
            else:
                args.append("-DENABLE_LIBTIFF=OFF")
        else:
            if "+tiff" in spec:
