...
class SagaGis(AutotoolsPackage, SourceforgePackage):
    ...
    # according to Spack recipe for gdal and https://gdal.org/drivers/raster/grib.html#raster-grib
    # grib has been a builtin library within gdal
    depends_on("gdal@2.3:+hdf5+netcdf")

    depends_on("gdal@2.3:2.4+hdf5+netcdf", when="@:7.2")
    ...
    def patch(self):
        if "+opencv" in self.spec:
            ...
            opencv_makefile = join_path("saga-gis", "src", "tools", "imagery", "imagery_opencv", "Makefile.am")
