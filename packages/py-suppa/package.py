...
class PySuppa(PythonPackage):
    ...
    version("2.4", sha256="a71b3017dd13ceb4b8b837dfae201eb3bc55fb11dfcb201434f6ff1e4733093c", url="https://github.com/comprna/SUPPA/archive/refs/tags/v2.4.tar.gz")
    ...
    # Build system
    depends_on("py-setuptools", type="build", when="@:2.4")
    ...
    def install_cli(self):
        ...
        if self.spec.satisfies("@:2.3"):
            src = join_path(src, "build", "lib", "SUPPA")
        ...
        perlexes = (
            "rmats_to_suppa_ids_SE_events.pl",
            "rmats_to_suppa_ids_A5_events.pl",
            "rmats_to_suppa_ids_A3_events.pl",
        )

        # Copy CLI script and its module dependencies
        for fname in pyexes + perlexes:
        ...
        for fname in pyexes:
            wrapper = join_path(self.prefix.bin, fname)
            with open(wrapper, "w") as f:
                f.write("#!/bin/bash\n")
                f.write("export PYTHONPATH=\"{0}:$PYTHONPATH\"\n".format(libexec_dir))
                f.write("exec python \"{0}\" \"$@\"\n".format(join_path(libexec_dir, fname)))
            chmod = which("chmod")
            chmod("+x", wrapper)

        for fname in perlexes:
            wrapper = join_path(self.prefix.bin, fname)
            with open(wrapper, "w") as f:
                f.write("#!/bin/bash\n")
                f.write("exec perl \"{0}\" \"$@\"\n".format(join_path(libexec_dir, fname)))
