...
class PyJaxlib(Package, CudaPackage):
    """XLA library for Jax.
    ...
    with default_args(type="build"):
        # .bazelversion
        depends_on("bazel@7.4.1", when="@0.5:")
        depends_on("bazel@6.5.0", when="@0.4.28:0.4.38")
    ...
    def setup_build_environment(self, env):
        wrapper_dir = join_path(self.stage.path, "spack-bazel-bin")
        mkdirp(wrapper_dir)
        uname_wrapper = join_path(wrapper_dir, "uname")
        if not os.path.exists(uname_wrapper):
            with open(uname_wrapper, "w") as fh:
                fh.write("#!/bin/sh\nexec /usr/bin/uname \"$@\"\n")
            os.chmod(uname_wrapper, 0o755)
        env.prepend_path("PATH", wrapper_dir)
        self._uname_wrapper_dir = wrapper_dir
    ...
    def install(self, spec, prefix):
        ...
        if hasattr(self, "_uname_wrapper_dir"):
            repo_path = f"{self._uname_wrapper_dir}:{os.environ.get('PATH', '')}"
            args.append(f"--bazel_options=--repo_env=PATH={repo_path}")

        if self.compiler.name == "gcc" and self.compiler.version < Version("12"):
            args.extend(
                [
                    "--bazel_options=--define=xnn_enable_avx512fp16=false",
                    "--bazel_options=--define=xnn_enable_avx512bf16=false",
                ]
            )

        python_exe(*args)

        wheels = sorted(glob.glob(join_path("dist", "*.whl")))
        if not wheels:
            raise InstallError("jaxlib wheel build did not produce any artifacts")

        pip_exe = Executable(spec["python"].command.path)
        pip_exe.add_default_env("PYTHONNOUSERSITE", "1")
        pip_exe.add_default_arg("-m")
        pip_exe.add_default_arg("pip")
        pip_args = PythonPipBuilder.std_args(self) + [f"--prefix={prefix}"]

        for whl in wheels:
            pip_exe(*(pip_args + [whl]))

    @run_after("install")
    def install_test(self):
        python_exe = Executable(self.spec["python"].command.path)
        with working_dir("spack-test", create=True):
            python_version = self.spec["python"].version.up_to(2)
            site_packages = join_path(self.prefix, "lib", f"python{python_version}", "site-packages")
            env = os.environ.copy()
            env["PYTHONPATH"] = os.pathsep.join(filter(None, [site_packages, env.get("PYTHONPATH", "")]))
            python_exe("-c", "import jaxlib", env=env)
