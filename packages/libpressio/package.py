...
class Libpressio(CMakePackage, CudaPackage):
    ...
    conflicts("+mgardx", when="+szauto", msg="SZ auto and MGARDx cause symbol conflicts with each other")
    conflicts(
        "~json",
        when="@0.57.0:+remote",
    ...
    def cmake_args(self):
        args = [
            ...
            self.define("LIBPRESSIO_CXX_VERSION", "11" if "+boost" in self.spec else self.lp_cxx_version()),
        ]
    ...
    def test(self):
        ...
        args.append("-S{}".format(join_path(self.test_suite.current_test_cache_dir, "test", "smoke_test")))
        args.append("-DCMAKE_PREFIX_PATH={};{}".format(self.spec["libstdcompat"].prefix, self.prefix))
