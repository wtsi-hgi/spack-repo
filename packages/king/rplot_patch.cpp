...
int plotAncestry(const char *prefix, int coreCount, bool pngFlag, const char *rpath)
{
    ...
    fprintf(fp, "rm(list=ls(all=TRUE))\n");
    fprintf(fp, "library(e1071)\n");
    fprintf(fp, "options(scipen = 999)\n");
    fprintf(fp, "prefix <- \"%s\"\n", prefix);
    fprintf(fp, "pc <- read.table(paste0(prefix, \"pc.txt\"), header = TRUE)\n");
    fprintf(fp, "phe <- read.table(paste0(prefix, \"_popref.txt\"), header = TRUE, stringsAsFactors = FALSE)\n");
    fprintf(fp, "pop_factor <- as.factor(phe$Population)\n");
    fprintf(fp, "unique_pop_factor <- unique(data.frame(Population = phe$Population, Factor = as.integer(pop_factor)))\n");
    fprintf(fp, "write.table(unique_pop_factor, file = \"king_popfactor.txt\", row.names = FALSE, quote = FALSE)\n");
    fprintf(fp, "Population <- c(pop_factor, rep(NA, (nrow(pc) - nrow(phe))))\n");
    fprintf(fp, "dat <- cbind(pc, Population)\n");
    fprintf(fp, "dat$fold <- 0\n");
    fprintf(fp, "set.seed(123)\n");
    fprintf(fp, "dat$fold[!is.na(dat$Population)] <- sample(1:5, sum(!is.na(dat$Population)), replace = T)\n");
    fprintf(fp, "numpc <- length(grep(\"PC\", colnames(pc)))\n");
    fprintf(fp, "numpc <- ifelse(numpc >= 10, 10, numpc)\n");
    fprintf(fp, "svm.mod <- as.formula(paste0(\"Population~\", paste0(\"PC\", 1:numpc, collapse = \"+\")))\n");
    fprintf(fp, "best.set <- c(-2, 0)\n");
    fprintf(fp, "step.mat <- cbind(c(3,3), c(2,2), c(1,1))\n");
    fprintf(fp, "para.one <- function(x) {\n");
    fprintf(fp, "  fold.index <- para[x, \"fold\"]\n");
    fprintf(fp, "  mod.svm <- svm(svm.mod, data = dat[dat$fold != fold.index, ], kernel = \"radial\", \n");
    fprintf(fp, "                 gamma = 10^para[x, \"gamma\"], cost = 10^para[x, \"cost\"], fitted = F, type = \"C\")\n");
    fprintf(fp, "  pred.svm <- predict(mod.svm, newdata = dat[dat$fold == fold.index, ])\n");
    fprintf(fp, "  return(sum(pred.svm == dat[dat$fold == fold.index, \"Population\"]))}\n");
    fprintf(fp, "for (i in 1:2) {\n");
    fprintf(fp, "  index <- dat$fold != 0\n");
    fprintf(fp, "  best.count <- 0\n");
    fprintf(fp, "  para.list <- NULL\n");
    fprintf(fp, "  for (j in 2:ncol(step.mat)) {\n");
    fprintf(fp, "    val1 <- best.set - step.mat[, j - 1]\n");
    fprintf(fp, "    val2 <- best.set + step.mat[, j - 1]\n");
    fprintf(fp, "    gamma.init <- seq(val1[1], val2[1], by = step.mat[1, j])\n");
    fprintf(fp, "    cost.init <- seq(val1[2], val2[2], by = step.mat[2, j])\n");
    fprintf(fp, "    para.set <- cbind(gamma = gamma.init, cost = rep(cost.init, each = length(gamma.init)))\n");
    fprintf(fp, "    para.all <- paste0(para.set[, 1], \"_\", para.set[, 2])\n");
    fprintf(fp, "    para.set <- para.set[!para.all %%in%% para.list, ]\n");
    fprintf(fp, "    para.list <- para.all\n");
    fprintf(fp, "    para = cbind(fold = rep(1:5, nrow(para.set)), gamma = rep(para.set[, \"gamma\"], 5), cost = rep(para.set[, \"cost\"], 5), count = NA)\n");
    fprintf(fp, "    para <- as.data.frame(para)\n");
    fprintf(fp, "    if (require(\"doParallel\", quietly = TRUE)) {\n");
    fprintf(fp, "      registerDoParallel(cores = %d)\n", coreCount);
    fprintf(fp, "      results <- foreach(para.index = 1:nrow(para), .combine = c) %%dopar%% {\n");
    fprintf(fp, "        para.one(para.index)}\n");
    fprintf(fp, "    } else {\n");
    fprintf(fp, "      results <- sapply(1:nrow(para), para.one)}\n");
    fprintf(fp, "    para[, \"count\"] <- unlist(results)\n");
    fprintf(fp, "    results <- aggregate(count ~ cost + gamma, para, sum)\n");
    fprintf(fp, "    results <- results[order(results$cost, results$gamma), ]\n");
    fprintf(fp, "    best.para <- results[which.max(results$count), ]\n");
    fprintf(fp, "    if (max(results$count) > best.count) {\n");
    fprintf(fp, "      best.set <- unlist(best.para[, c(\"gamma\", \"cost\")])\n");
    fprintf(fp, "      best.count <- max(results$count)}\n");
    fprintf(fp, "    }\n");
    fprintf(fp, "  mod_svm <- svm(formula = svm.mod, data = dat[index, ], kernel = \"radial\", gamma = 10^best.set[1], \n");
    fprintf(fp, "                 cost = 10^best.set[2], probability = T, fitted = T, type = \"C\")\n");
    fprintf(fp, "  pred_svm <- predict(mod_svm, newdata = dat[, paste0(\"PC\", 1:numpc)], probability = T)\n");
    fprintf(fp, "  suspi_pop <- index & pred_svm != dat$Population\n");
    fprintf(fp, "  if (sum(suspi_pop) == 0) {\n");
    fprintf(fp, "    break\n");
    fprintf(fp, "  } else {\n");
    fprintf(fp, "    dat$fold[suspi_pop] <- 0\n");
    fprintf(fp, "    set.seed(123)\n");
    fprintf(fp, "    dat$fold[dat$fold != 0] <- sample(1:5, sum(dat$fold != 0), replace = T)\n");
    fprintf(fp, "    step.mat <- step.mat[, -1]\n");
    fprintf(fp, "  }\n");
    fprintf(fp, "}\n");
    fprintf(fp, "class.prob <- attr(pred_svm, \"probabilities\")\n");
    fprintf(fp, "print(paste(\"Prepare the summary file, starts at\", date()))\n");

    fprintf(fp, "Summary <- function(v) {\n");
    fprintf(fp, "v.whichmax <- which.max(v)\n");
    fprintf(fp, "v2.whichmax <- which.max(v[-v.whichmax])\n");
    fprintf(fp, "v.2ndwhichmax <- v2.whichmax + (v2.whichmax>=v.whichmax)\n");
    fprintf(fp, "c(v.whichmax, v[v.whichmax], v.2ndwhichmax, v[v.2ndwhichmax])\n");
    fprintf(fp, "}\n");
    fprintf(fp, "valid <- dat[,\"AFF\"]==2\n");
    fprintf(fp, "class.prob.valid <- class.prob[valid,]\n");
    fprintf(fp, "maxed <- apply(class.prob.valid, 1, Summary)\n");
    fprintf(fp, "popcount <- ncol(class.prob)\n");
    fprintf(fp, "popnames <- colnames(class.prob)\n");
    fprintf(fp, "Ancestry.1 <- popnames[maxed[1,]]\n");
    fprintf(fp, "pred_class <- Ancestry.1\n");
    fprintf(fp, "Pr.1 <- maxed[2,]\n");
    fprintf(fp, "for(i in 1:length(Pr.1)) if(Pr.1[i]<=0.65){\n");
    fprintf(fp, "x <- sort(class.prob.valid[i,], decreasing=TRUE)\n");
    fprintf(fp, "temp <- (1:popcount)[cumsum(x)>0.65][1]\n");
    fprintf(fp, "pred_class[i] <- paste(names(x)[1:temp], collapse=\";\")}\n");
    fprintf(fp, "pred.out <- cbind(dat[valid, c(\"FID\", \"IID\", \"PC1\", \"PC2\")], Ancestry.1, round(Pr.1,4),popnames[maxed[3,]], round(maxed[4,],4), pred_class)\n");
    fprintf(fp, "colnames(pred.out) <- c(\"FID\", \"IID\", \"PC1\", \"PC2\", \"Anc_1st\", \"Pr_1st\", \"Anc_2nd\",  \"Pr_2nd\", \"Ancestry\")\n");
    ...
    fprintf(fp, "print(paste(\"Results are saved to\", paste0(prefix, \"_InferredAncestry.txt\"), date()))\n");
    fprintf(fp, "print(paste(\"Generate plots\", date()))\n");
    fprintf(fp, "pred.out$Ancestry <- as.character(pred.out$Ancestry)\n");
    fprintf(fp, "pred.out$Ancestry[grep(\";\", pred.out$Ancestry)] <- \"Missing\"\n");
    fprintf(fp, "Palette <- c(\"#1F78B4\", \"#33A02C\", \"#E31A1C\", \"#FF7F00\", \"#6A3D9A\", \"#B15928\", \"#A6CEE3\", \n");
    fprintf(fp, "             \"#B2DF8A\", \"#FB9A99\", \"#FDBF6F\", \"#CAB2D6\", \"#FFFF99\", \"#999999\")\n");
    fprintf(fp, "train.phe <- dat[dat$AFF == 1, ]\n");
    ...
    if (pngFlag)
        fprintf(fp, "png(paste0(prefix, \"_ancestryplot.png\"), width=854, height=480)\n");
    else
    ...
    fprintf(fp, "if (\"Missing\" %%in%% unique(pred.out$Ancestry)) {\n");
    fprintf(fp, "  legend.group <- c(sort(unique(pred.out$Ancestry)[unique(pred.out$Ancestry) != \"Missing\"]), \"Missing\")\n");
    fprintf(fp, "  all.color <- unique(pred.colors)[order(unique(pred.out$Ancestry))]\n");
    fprintf(fp, "  legend.color <- c(all.color[!all.color %%in%% c(\"#999999\")], \"#999999\")\n");
    fprintf(fp, "} else {\n");
    fprintf(fp, "  legend.group <- sort(unique(pred.out$Ancestry))\n");
    fprintf(fp, "  legend.color <- unique(pred.colors)[order(unique(pred.out$Ancestry))]\n");
    fprintf(fp, "}\n");
    ...
    if (!pngFlag) {
        ...
        fprintf(fp, "  legend(\"topright\", legend = legend.group, col = legend.color, pch = 16, cex = 1)\n");
    }
    fprintf(fp, "  par(mfrow = c(2, ncols))\n");
    fprintf(fp, "  for (i in legend.group) {\n");
    ...
    fprintf(fp, "         xlim = c(x.low, x.high), ylim = c(y.low, y.high), xlab = \"PC1\", ylab = \"PC2\", \n");
    fprintf(fp, "         main = paste0(i, \" (N=\", nrow(subdata), \")\"))\n");
    fprintf(fp, "  }\n");
    if (!pngFlag) {
        fprintf(fp, "  par(mfrow = c(1, 1))\n");
        fprintf(fp, "  plot(train.phe$PC1, train.phe$PC2, col = train.colors, xlim = c(x.low, x.high), \n");
        fprintf(fp, "       ylim = c(y.low, y.high), xlab = \"PC1\", ylab = \"PC2\", main = \"Populations in Reference\", pch = 16)\n");
        fprintf(fp, "  legend(\"topright\", legend = sort(unique(train.phe$Population)), col = unique(train.colors)[order(unique(train.phe$Population))], \n");
        fprintf(fp, "         pch = 16, cex = 1)\n");
    }
    fprintf(fp, "} else {\n");
    if (!pngFlag) {
        fprintf(fp, "  p <- ggplot(pred.out, aes(x = PC1, y = PC2))\n");
        fprintf(fp, "  p <- p + geom_point(aes(colour = factor(Ancestry, levels = legend.group))) + \n");
        fprintf(fp, "    xlim(x.low, x.high) + ylim(y.low, y.high) + labs(color = \"\") + scale_colour_manual(values = legend.color) + \n");
        fprintf(fp, "    ggtitle(paste(\"Inferred Populations as Ancestry in\", prefix))\n");
        fprintf(fp, "  print(p)\n");
    }
    fprintf(fp, "  labels <- sapply(legend.group, function(x) paste0(x, \" (N=\", sum(pred.out$Ancestry == x), \")\"))\n");
    fprintf(fp, "  p <- ggplot(pred.out, aes(x = PC1, y = PC2, colour = factor(Ancestry, levels = legend.group))) + \n");
    fprintf(fp, "    scale_color_manual(values = legend.color) + theme(legend.position = \"none\")\n");
    fprintf(fp, "  p <- p + geom_point() + xlim(x.low, x.high) + ylim(y.low, y.high) + \n");
    fprintf(fp, "    facet_wrap(~factor(Ancestry, levels = legend.group, labels = labels), ncol = min(3, ncols))\n");
    fprintf(fp, "  print(p)\n");
    if (!pngFlag) {
        fprintf(fp, "  p <- ggplot(train.phe, aes(x = PC1, y = PC2))\n");
        fprintf(fp, "  p <- p + geom_point(aes(colour = factor(Population, levels = sort(unique(Population))))) + \n");
        fprintf(fp, "    xlim(x.low, x.high) + ylim(y.low, y.high)\n");
        fprintf(fp, "  p <- p + labs(color = \"\") + scale_colour_manual(values = unique(train.colors)[order(unique(train.phe$Population))]) + \n");
        fprintf(fp, "    ggtitle(\"Populations in Reference\")\n");
        fprintf(fp, "  print(p)\n");
    }
    fprintf(fp, "}\n");
    fprintf(fp, "dev.off()\n");
    ...
    case 0:  
        if (pngFlag) {
            ...
            printf("Ancestry plots are generated in %s_ancestryplot.png\n", (const char*)prefix);
        }
        else {
            sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_ancestryplot.ps", (const char*)prefix);
            system(command);
            printf("Ancestry populations are inferred as in %s_InferredAncestry.txt\n", (const char*)prefix);
...
//fprintf(fp, "pred.out <- cbind(dat[, c(\"FID\", \"IID\", \"PC1\", \"PC2\", \"AFF\")], orders.class[, 1], \n");
//fprintf(fp, "                  round(orders.probs[, 1], 4), orders.class[, 2], round(orders.probs[, 2], 4), pred_class)\n");
...
void plotROH(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_rohplot.ps", (const char*)prefix);
               system(command);
...
void plotMIerror(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_MIerrorplot.ps", (const char*)prefix);
               system(command);
...
void plotUniqueFamily(const char *prefix, int degree, const char *analysis, const char *rpath)
{
   ...
   if(!CheckRout(scriptfile)){
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s.ps", (const char*)namefile);
      system(command);
...
void plotDuplicate(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("--duplicate is done but R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_duplicateplot.ps", prefix);
               system(command);
...
void plotCluster(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("--cluster is done but R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_clusterplot.ps", (const char*)prefix);
               system(command);
...
void plotSplitped(const char *prefix, const char *rpath)
{
   ...
   fprintf(fp, "    max.num <- max(align.pedigree(pedAll[toString(f)])$n)\n");
   fprintf(fp, "    size <- ifelse(max.num > 15, 42/max.num, 2.8)\n");
   fprintf(fp, "    plot(pedAll[toString(f)], cex=0.5, symbolsize = size)}\n");
   fprintf(fp, "dev.off()\n");
   fprintf(fp, "rm(list=ls(all=TRUE))\n");
   fclose(fp);
   char command[256];
   ...
   switch(error){
      ...
      case 1:  printf("R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_pedplot.ps", (const char*)prefix);
               system(command);
...
void plotBuild(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("--build is done but R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_buildplot.ps", (const char*)prefix);
               system(command);
...
void plotHEreg(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_herplot.ps", (const char*)prefix);
      system(command);
...
void plotPopROH(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_poprohplot.ps", (const char*)prefix);
      system(command);
...
void plotROHforQT(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_mthomoplot.ps", (const char*)prefix);
      system(command);
...
void plotROHmapping(const char *prefix, const char *stratName, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_%splot.ps",
         (const char*)prefix, (const char*)postfix);
      system(command);
...
void plotPopDist(const char *prefix, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_popdistplot.ps", (const char*)prefix);
      system(command);
...
void plotIBDmapping(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_ibdmapplot.ps", (const char*)prefix);
      system(command);
...
void plotNPL(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_nplplot.ps", (const char*)prefix);
      system(command);
...
void plotAUCmapping(const char *prefix, int SEXCHR, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_aucmapplot.ps", (const char*)prefix);
      system(command);
...
void plotRelationship(const char *prefix, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_relplot.ps", (const char*)prefix);
      system(command);
...
void plotIBDSeg(const char *prefix, const char *rpath)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_ibd1vsibd2.ps", (const char*)prefix);
      system(command);
      printf("IBD1 vs IBD2 plot is generated in %s_ibd1vsibd2.pdf\n", (const char*)prefix);
   }
}

void plotGenderError(const char *prefix, IntArray & plotx, Vector & ploty, IntArray & plotz, double xHeterozygosity, int gendererrorCount, bool pngFlag, const char *rpath)
{
   ...
   if(pngFlag)
       genderplot.Add("_gender_autoplot.png");
   else
   ...
   if (pngFlag)
       fprintf(fp, "png(\"%s\", width=640, height=480)\n", (const char*)genderplot);
   else
   ...
   if(error == 2)
      ...
      printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
       ...
       if(pngFlag)
           printf("Gender plot are generated in %s\n", (const char*)genderplot);
       else {
           sprintf(command, "ps2pdf -sPAPERSIZE=letter %s", (const char*)genderplot);
           system(command);
...
void plotPopStructure(const char *prefix, int projectFlag, bool pngFlag, const char *rpath)
{
   String scriptfile=prefix;
   ...
   fprintf(fp, "rm(list=ls(all=TRUE))\n");
   if (pngFlag)
       fprintf(fp, "png(\"%s_pcplot.png\", width=640, height=480)\n", (const char*)prefix);
   else
   ...
   if(error == 2)
      ...
      printf("  Please check %sout for details.\n\n", (const char*)scriptfile);
       ...
       if (pngFlag) 
           printf("Population structure plot is generated in %s_pcplot.png\n", (const char*)prefix);
       else{
           sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_pcplot.ps", (const char*)prefix);
           system(command);
...
void plotHetConcvsIBD2(const char *prefix)
{
   ...
   if(error == 2)
      ...
      sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_ibd2plot.ps", (const char*)prefix);
      system(command);
...
int plotAncestry(const char *prefix, const char *rpath)
{
   ...
   switch(error){
      ...
      case 1:  printf("R code %s failed.\n", (const char*)scriptfile);
               printf("  Please check %sout for details.\n", (const char*)scriptfile);
               break;
      case 0:  sprintf(command, "ps2pdf -sPAPERSIZE=letter %s_ancestryplot.ps", (const char*)prefix);
               system(command);
               printf("Ancestry populations are inferred as in %s_InferredAncestry.txt\n", (const char*)prefix);
               printf("Ancestry plots are generated in %s_ancestryplot.pdf\n", (const char*)prefix);
               break;
...
fprintf(fp, "pc <- read.table(paste0(prefix, \"pc.txt\"), header = TRUE)\n");
fprintf(fp, "phe <- read.table(paste0(prefix, \"_popref.txt\"), header = TRUE)\n");
fprintf(fp, "Population <- c(as.character(phe$Population), rep(NA, (nrow(pc)-nrow(phe))) )\n");
fprintf(fp, "dat <- cbind(pc,Population)\n");
fprintf(fp, "dat$fold <- 0\n");
fprintf(fp, "set.seed(123)\n");
fprintf(fp, "dat$fold[!is.na(dat$Population)] <- sample(1:5, sum(!is.na(dat$Population)), replace=T)\n");
...
fprintf(fp, "numpc <- ifelse(numpc >= 10, 10, numpc)\n");
fprintf(fp, "svm.mod <- as.formula(paste0(\"Population~\", paste0(\"PC\", 1:numpc, collapse = \"+\")))\n");
fprintf(fp, "para_best = c(-2, 0)\n");
fprintf(fp, "step_cv0 = c(3, 3)\n");
fprintf(fp, "step_cv1st = c(2, 2)\n");
fprintf(fp, "step_cv2nd = c(1, 1)\n");
fprintf(fp, "step_mat <- cbind(step_cv0, step_cv1st, step_cv2nd)\n");
fprintf(fp, "para_one <- function(x) {\n");
fprintf(fp, "    fold.index <- para[x, \"fold\"]\n");
fprintf(fp, "    mod.svm <- svm(svm.mod, data = dat[dat$fold != fold.index, ], kernel = \"radial\", \n");
fprintf(fp, "        gamma = 10^para[x, \"gamma\"], cost = 10^para[x, \"cost\"], fitted = F)\n");
fprintf(fp, "    pred.svm <- predict(mod.svm, newdata = dat[dat$fold == fold.index, ])\n");
fprintf(fp, "    return(sum(pred.svm == dat[dat$fold == fold.index, \"Population\"]))\n");
fprintf(fp, "}\n");
fprintf(fp, "for (i in 1:2) {\n");
fprintf(fp, "  index <- dat$fold != 0\n");
fprintf(fp, "  perf_best <- 0\n");
fprintf(fp, "  mark_para <- NULL\n");
fprintf(fp, "  for(j in 2:ncol(step_mat)) {\n");
fprintf(fp, "    a0 <- para_best - step_mat[, j - 1]\n");
fprintf(fp, "    b0 <- para_best + step_mat[, j - 1];\n");
fprintf(fp, "    gamma0 <- seq(a0[1], b0[1], by = step_mat[1, j]);\n");
fprintf(fp, "    cost0 <- seq(a0[2], b0[2], by = step_mat[2, j]);\n");
fprintf(fp, "    para0 <- cbind(gamma = gamma0, cost = rep(cost0, each = length(gamma0)));\n");
fprintf(fp, "    mark0 <- paste0(para0[, 1], \"_\", para0[, 2]);\n");
fprintf(fp, "    para0 <- para0[!mark0 %%in%% mark_para, ];\n");
fprintf(fp, "    mark_para <- mark0;\n");
fprintf(fp, "    para = cbind(fold=rep(1:5,nrow(para0)),gamma=rep(para0[,\"gamma\"],5), cost=rep(para0[,\"cost\"],5),count=NA)\n");
fprintf(fp, "    para <- as.data.frame(para)\n");
fprintf(fp, "    if (require(\"doParallel\", quietly = TRUE)){\n");
fprintf(fp, "      numCores <- detectCores()\n");
fprintf(fp, "      registerDoParallel(cores = round(numCores/2))\n");
fprintf(fp, "      results <- foreach(para.index  = 1:nrow(para), .combine = c) %%dopar%% {para_one(para.index)}\n");
fprintf(fp, "    } else {\n");
fprintf(fp, "      results <- sapply(1:nrow(para), para_one)\n");
fprintf(fp, "    }\n");
fprintf(fp, "    para[,\"count\"] <- unlist(results)\n");
fprintf(fp, "    results <- aggregate(count ~ cost + gamma, para, sum)\n");
fprintf(fp, "    results <- results[order(results$cost, results$gamma),]\n");
fprintf(fp, "    best.para <- results[which.max(results$count),]\n");
fprintf(fp, "    if(max(results$count) > perf_best) {\n");
fprintf(fp, "      para_best <- unlist(best.para[,c(\"gamma\", \"cost\")])\n");
fprintf(fp, "      perf_best <- max(results$count)\n");
fprintf(fp, "    }\n");
fprintf(fp, "  }\n");
fprintf(fp, "  # SVM\n");
fprintf(fp, "  mod_svm <- svm(formula = svm.mod, data = dat[index, ], kernel = \"radial\", gamma = 10^para_best[1], cost = 10^para_best[2], probability = T, fitted = T);\n");
fprintf(fp, "  pred_svm <- predict(mod_svm, newdata = dat[, paste0(\"PC\", 1:numpc)], probability = T)\n");
fprintf(fp, "  suspi_pop <- index & pred_svm != dat$Population\n");
fprintf(fp, "  if(sum(suspi_pop) == 0) {\n");
fprintf(fp, "    break;\n");
...
fprintf(fp, "    set.seed(123)\n");
fprintf(fp, "    dat$fold[dat$fold != 0] <- sample(1:5, sum(dat$fold != 0), replace = T)\n");
fprintf(fp, "    step_cv0 <- step_cv1st\n");
fprintf(fp, "  }\n");
fprintf(fp, "}\n");
fprintf(fp, "dat$Ancestry <- as.character(pred_svm)\n");
fprintf(fp, "class.prob <- attr(pred_svm, \"probabilities\")\n");
fprintf(fp, "print(paste(\"Prepare the summary file, starts at\", date()))\n");
fprintf(fp, "orders <- t(apply(class.prob, 1, function(x) order(x,decreasing=T)))\n");
fprintf(fp, "orders.class <- t(apply(orders, 1, function(x) colnames(class.prob)[x]))\n");
...
fprintf(fp, "temp <- apply(check.cumsum, 1, function(x) which(x > 0.65)[1])\n");
fprintf(fp, "pred_class <- sapply(1:length(temp), function (x) paste(orders.class[x, 1:as.numeric(temp[x])], collapse = \";\"))\n");
fprintf(fp, "pred_prob <- sapply(1:length(temp), function (x) paste(round(orders.probs[x, 1:as.numeric(temp[x])], 4), collapse = \";\"))\n");
fprintf(fp, "pred.out <- cbind(dat[, c(\"FID\", \"IID\", \"PC1\", \"PC2\",\"AFF\")], pred_class, pred_prob, orders.class[, 1], orders.class[, 2], round(orders.probs[, 1], 4), round(orders.probs[, 2], 4))\n");
fprintf(fp, "colnames(pred.out) <- c(\"FID\", \"IID\", \"PC1\",\"PC2\",\"AFF\", \"Ancestry\", \"Pr_Anc\", \"Anc_1st\", \"Anc_2nd\", \"Pr_1st\", \"Pr_2nd\")\n");
fprintf(fp, "pred.out <- pred.out[pred.out$AFF==2,]\n");
fprintf(fp, "pred.out <- pred.out[,!colnames(pred.out)%%in%% c(\"AFF\")]\n");
fprintf(fp, "pred.out.results <- pred.out\n");
fprintf(fp, "pred.out.results$PC1 <- format(pred.out.results$PC1, scientific=F)\n");
fprintf(fp, "pred.out.results$PC2 <- format(pred.out.results$PC2, scientific=F)  \n");
fprintf(fp, "print(paste(\"summary file is ready \", date()))\n");
fprintf(fp, "write.table(pred.out.results, paste0(prefix, \"_InferredAncestry.txt\"), sep = \"	\", quote = FALSE, row.names = FALSE)\n");
fprintf(fp, "print(paste(\"Results are saved to\", paste0(prefix, \"_InferredAncestry.txt\")))\n");
fprintf(fp, "print(\"Generate plots\")\n");
fprintf(fp, "pred.out$Ancestry <- as.character(pred.out$Ancestry)\n");
fprintf(fp, "#pred.out$Ancestry[pred.out$Pr_1st <= 0.65] <- \"Missing\"\n");
fprintf(fp, "pred.out$Ancestry[grep(\";\", pred.out$Ancestry)] <- \"Missing\"\n");
fprintf(fp, "Palette <- c(\"#1F78B4\",\"#33A02C\",\"#E31A1C\",\"#FF7F00\",\"#6A3D9A\",\"#B15928\",\"#A6CEE3\",\"#B2DF8A\",\"#FB9A99\",\"#FDBF6F\",\"#CAB2D6\",\"#FFFF99\",\"#999999\")\n");
fprintf(fp, "train.phe <- dat[dat$AFF==1,]\n");
fprintf(fp, "train.groups <- unique(train.phe$Population)\n");
...
fprintf(fp, "  p <- p + geom_point(aes(colour = factor(Ancestry, levels = legend.group))) +\n");
fprintf(fp, "    xlim(x.low, x.high) + ylim(y.low, y.high) + labs(color = \"\") + \n");
fprintf(fp, "    scale_colour_manual(values = legend.color) +\n");
fprintf(fp, "    ggtitle(paste(\"Inferred Populations as Ancestry in\", prefix))\n");
fprintf(fp, "  print(p)\n");
fprintf(fp, "  labels <- sapply(legend.group, function(x) paste0(x, \" (N=\", sum(pred.out$Ancestry == x), \")\"))\n");
fprintf(fp, "  p <- ggplot(pred.out, aes(x = PC1, y = PC2, colour = factor(Ancestry, levels = legend.group))) +\n");
