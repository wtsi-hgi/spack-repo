...
try:
    ...
    def get_cmake_prefix_path(pkg):
        prefixes = []
        for dep in pkg.spec.traverse(deptype=("build", "run"), root=False):
...
try:
    from spack.package import CompilerPackage  # type: ignore[attr-defined]
except ImportError:
    class CompilerPackage(Package):
        """Fallback for older Spack releases without CompilerPackage."""

        pass

if "90a" not in CudaPackage.cuda_arch_values:
    CudaPackage.cuda_arch_values = CudaPackage.cuda_arch_values + ("90a",)

for _directive in getattr(CudaPackage, "_directives_to_be_executed", []):
    _closure = getattr(_directive, "__closure__", None)
    if not _closure:
        continue
    string_contents = [cell.cell_contents for cell in _closure if isinstance(cell.cell_contents, str)]
    if "cuda_arch" not in string_contents:
        continue
    allowed_cell = next(
        (cell for cell in _closure if hasattr(cell.cell_contents, "feature_values")), None
    )
    if allowed_cell is None:
        continue
    allowed = allowed_cell.cell_contents
    for value_set in allowed.sets:
        if "90" in value_set:
            value_set.add("90a")
            break
    else:
        allowed.sets.append(set(("90a",)))
    allowed.feature_values = tuple(
        value for value_set in allowed.sets for value in value_set
    )
    if "90a" not in allowed.feature_values:
        allowed.feature_values = allowed.feature_values + ("90a",)
# cleanup temporary variables
del _directive, _closure, string_contents, allowed_cell
