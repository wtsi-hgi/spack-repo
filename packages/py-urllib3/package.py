...
class PyUrllib3(PythonPackage):
    ...
    depends_on("python@3.8:", when="@2.1:2.5", type=("build", "run"))
    depends_on("py-hatchling@1.6:1", when="@2:", type="build")
    depends_on("py-hatch-vcs@0.4:0.5", when="@2.6:", type="build")
    depends_on("py-setuptools-scm@8:9", when="@2.6:", type="build")
    ...
    def patch(self):
        if self.spec.satisfies("@2.6.3:"):
            filter_file(r'^\s*"Programming Language :: Python :: 3\.14",\n', "", "pyproject.toml")
            filter_file(
                r'^\s*"Programming Language :: Python :: Free Threading :: 2 - Beta",\n',
                "",
                "pyproject.toml",
            )
