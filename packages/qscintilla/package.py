...
class Qscintilla(QMakePackage):
    ...
    def fix_install_path(self):
        ...
        mkdirp(self.prefix.translations)
        mkdirp(self.prefix.mkspecs.features)
        makefile = FileFilter(join_path(self.build_directory, "Makefile"))
        makefile.filter(r"\$\(INSTALL_ROOT\).*{}".format(self.spec["qmake"].dag_hash()), "$(INSTALL_ROOT)")
    ...
    def make_qsci_python(self):
        ...
        with working_dir(join_path(self.stage.source_path, "Python")):
            ...
            sip_inc_dir = join_path(self.spec[py_pyqtx].package.module.python_platlib, pyqtx, "bindings")
            ...
            with open("pyproject.toml", "a") as tomlfile:
                ...
                link_qscilibs = "LIBS += -L" + join_path(self.stage.source_path, "src") + " -lqscintilla2_" + qtx
                inc_qsciheaders = "INCLUDEPATH += " + join_path(self.stage.source_path, "src")
                tomlfile.write(
                    f'\n[tool.sip.builder]\nqmake-settings = \
                    ["QT += widgets", "QT += printsupport", "{inc_qsciheaders}", "{link_qscilibs}"]\n'
                )
