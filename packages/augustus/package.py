...
class Augustus(MakefilePackage):
    ...
    def edit(self, spec, prefix):
        ...
        with working_dir(join_path("auxprogs", "filterBam", "src")):
            ...
            # Robustly link BamTools by searching both lib and lib64 and linking by -l
            makefile.filter(
                r"LIBS\s*=.*",
                "LIBS = -Wl,-rpath,$(BAMTOOLS)/lib -Wl,-rpath,$(BAMTOOLS)/lib64 "
                "-L$(BAMTOOLS)/lib -L$(BAMTOOLS)/lib64 -lbamtools -lz",
            )
        with working_dir(join_path("auxprogs", "bam2hints")):
            ...
            makefile.filter(
                r"LIBS\s*=.*",
                f"LIBS = -Wl,-rpath,{bamtools}/lib -Wl,-rpath,{bamtools}/lib64 "
                f"-L{bamtools}/lib -L{bamtools}/lib64 -lbamtools -lz",
            )

        if self.version < Version("3.4.0"):
            # Skip building bam2wig on older releases: it depends on legacy bam.h API removed in modern htslib
            with working_dir("auxprogs"):
                aux_makefile = FileFilter("Makefile")
                aux_makefile.filter(r"^\s*cd bam2wig; make;\s*$", "# skipped bam2wig; legacy API")
                aux_makefile.filter(r"^\s*cd bam2wig; make clean;\s*$", "# skipped bam2wig clean")
            ...
            with working_dir(join_path("auxprogs", "bam2wig")):
                ...
                # htslib headers live under include/htslib; provide that so '#include "sam.h"' resolves
                makefile.filter("HTSLIB=.*$", f"HTSLIB={htslib}/include/htslib")

                # Newer htslib lacks bam.h; drop that include to avoid missing header
                src = FileFilter("bam2wig.c")
                # Remove legacy bam.h include (replaced by sam.h in modern htslib)
                src.filter(r'^\s*#include\s+"bam\.h"\s*$', "")
