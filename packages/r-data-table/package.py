...
class RDataTable(RPackage):
	...
	def setup_build_environment(self, env):
		# Ensure legacy allocation macros (Calloc/Realloc/Free) are defined
		# by including RS.h explicitly when building with newer R
		env.append_flags("CPPFLAGS", "-include R_ext/RS.h")
		# data.table uses OpenMP; ensure flags are present
		env.append_flags("CFLAGS", "-fopenmp")
		env.append_flags("LDFLAGS", "-fopenmp")

	def patch(self):
		# Inject a compatibility header to restore Calloc/Realloc/Free macros
		# using the checked-allocators from R_ext/Memory.h on newer R toolchains.
		import os
		src_dir = os.path.join('src')
		os.makedirs(src_dir, exist_ok=True)
		compat_h = os.path.join(src_dir, 'spack_compat.h')
		compat_contents = (
			"#ifndef SPACK_DATA_TABLE_COMPAT_H\n"
			"#define SPACK_DATA_TABLE_COMPAT_H\n"
			"#include <R.h>\n"
			...
			"#endif\n"
			"#endif /* SPACK_DATA_TABLE_COMPAT_H */\n"
		)
		with open(compat_h, 'w', encoding='utf-8') as f:
			f.write(compat_contents)
		for fname in [os.path.join(src_dir, 'Makevars'), os.path.join(src_dir, 'Makevars.in')]:
			if os.path.exists(fname):
				with open(fname, 'a', encoding='utf-8') as f:
					f.write('\nPKG_CPPFLAGS += -include spack_compat.h\n')
			else:
				with open(fname, 'w', encoding='utf-8') as f:
					f.write('PKG_CPPFLAGS += -include spack_compat.h\n')
